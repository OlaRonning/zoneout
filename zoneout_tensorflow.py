...
class ZoneoutWrapper(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    if isinstance(self.state_size, tuple):
      if self.is_training:
          new_state = tf.contrib.rnn.LSTMStateTuple(*((1 - state_part_zoneout_prob) * tf.python.nn_ops.dropout(
          ...
          new_state = tf.contrib.rnn.LSTMStateTuple(*(state_part_zoneout_prob * state_part + (1 - state_part_zoneout_prob) * new_state_part
                            for new_state_part, state_part, state_part_zoneout_prob in zip(new_state, state, self._zoneout_prob)))
    else:
      if self.is_training:
